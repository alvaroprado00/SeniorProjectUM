...
class CategoryBadges extends GetView<ActiveUserController> {
  ...
  Widget build(BuildContext context) {
    ...
    final args = ModalRoute.of(context)!.settings.arguments as CategoryBadgesArgs;
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        backgroundColor: tertiaryColor,
        elevation: 0,
        title: Text('My Badges', style: getSubheadingStyleBlue(),),
...
class CategoryBadgesArgs{
  ...
  const CategoryBadgesArgs({required Map<String, String> this.coursesInCategory, required Category this.category});
...
Widget getAllBadges({required Map<String, String> coursesInCategory, required BuildContext context, required List<Badge> userBadges}){
  ...
  for (Badge b in userBadges){
    if(coursesInCategory.containsKey(b.courseID)){
      childrenOfRow.add(getIconButtonForBadge(badge: b, nameOfCourse: coursesInCategory[b.courseID]!, size: 0.12*heightOfScreen, context: context));
    }
  }

  if(childrenOfRow.isEmpty){
    return Center(child: Text('No badges Earned yet', style: getSubheadingStyleBlue(),),);
  }
