...
class FillInTheBlanksQuestion extends Question {
  ...
  final List<String> options;
   ...
   FillInTheBlanksQuestion(
      {required int number,
        ...
        }): super(number:number, typeOfQuestion: typeOfQuestion, longFeedback: longFeedback);

  static getFillInTheBlanksQuestion(){
    return FillInTheBlanksQuestion(number: 2, typeOfQuestion:TypeOfQuestion.fillInTheBlanks, longFeedback:'', text: 'This is an example text with one blank coming now X and another one coming rigth after X', solution: {1:'hola',2:'adios'}, options: ['hola','adios','que tal']);
  }
  ...
  factory FillInTheBlanksQuestion.fromJson(Map<String, dynamic> json){
    return FillInTheBlanksQuestion(
      ...
      solution: Map.from(json['solution'].map((key,value){
        return MapEntry(
          int.parse(key),
          value
        );
