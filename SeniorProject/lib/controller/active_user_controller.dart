...
class ActiveUserController extends GetxController{
  ...
  bool isCurrentCourse({required String courseID}){
    if(this.currentCourse.value!=null && this.currentCourse.value!.courseID==courseID){
      return true;
    }else{
      return false;
    }
  ...
  unsaveCourse({required String courseID}){
    ...
    UserController.updateSimpleUserField( nameOfField: 'coursesSaved', field: this.coursesSaved );
  }
  ...
  Future<void> updateCurrentCourse() async {
    ...
    return UserController.updateComplexUserField(nameOfField: 'currentCourse', field: cc);
  }
  

  Future updateProfilePictureActive({required String newPicture}){
    this.profilePictureActive.value=newPicture;
    return UserController.updateSimpleUserField(nameOfField: 'profilePictureActive', field: newPicture);
  }
  ...
  Future saveCompletedCourse() async {
    ...
    //attribute to null
    if(this.currentCourse.value!=null && this.currentCourse.value!.courseID==completedCourse.courseID){
      await this.removeCurrentCourse();
    }

    //After all of this we need to update the user in the DB
    try{
      await UserController.updateComplexListUserField(nameOfField: 'completedCourses', field: this.completedCourses.value);
  ...
  Future<void> updateLevel() async {
    await UserController.updateComplexUserField(nameOfField: 'level', field: this.level.value);
  }
  /**
  ...
  Future<void> addNewAvatar() async {
    ...
    await UserController.updateSimpleUserField(nameOfField: 'collectedAvatars', field: this.collectedAvatars.value);
  }
  ...
  Future<void> removeCurrentCourse() async {
    ...
    await UserController.updateSimpleUserField(nameOfField: 'currentCourse', field: null);
  }
